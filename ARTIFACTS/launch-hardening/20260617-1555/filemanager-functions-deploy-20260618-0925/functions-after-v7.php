	...
	(function () {
		...
		function hideRatingClaims(root) {
			Array.prototype.slice.call(root.querySelectorAll('*')).forEach(function (element) {
				...
				if (/(300\+|Hundratals|Nöjda kunder|120\+|24h|franchisetagare)/i.test(text)) return;
		...
		function patchTrustText(root) {
			Array.prototype.slice.call(root.querySelectorAll('*')).forEach(function (element) {
				...
				if (/^300\+$/.test(text)) {
					element.textContent = 'Hundratals';
					...
					element.textContent = 'Hundratals nöjda kunder sedan 2008';
				}
			});
		}
